...
// ✅ Always use localhost to avoid GitHub DNS issues
...
(async function runTests() {
  ...
  try {
    ...
    // Locate input and submit button
    ...
    const resultText = await driver.wait(
      until.elementLocated(By.css(".result-box p")),
      3000
    ).getText();
    ...
    // 🔁 Re-locate elements after UI re-renders
    ...
    const xssError = await driver.wait(
    ...
    await input2.clear();
    await submitButton2.click();
    const emptyError = await driver.wait(
      until.elementLocated(By.css(".error")),
      3000
    ).getText();
